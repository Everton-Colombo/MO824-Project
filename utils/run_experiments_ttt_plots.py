...
def run_single_instance(config_bundle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    solver_name = config_bundle['solver_name']
    info = config_bundle['info']
    paths = config_bundle['paths']
    
    base_strategy_config = config_bundle['strategy_config']
    ...
    result_fail = {
        ...
        'Phased': 'Yes' if base_strategy_config['phased_optimization'] is not None else 'No',
        'Status': 'Error',
    ...
    phased_opt = None
    if base_strategy_config['phased_optimization'] is not None:
    ...
    try:
        ...
        return {
            ...
            'Phased': 'Yes' if phased_opt is not None else 'No',
            'Status': 'Success',
...
def main():
    print("Iniciando experimento TTT (4x4x50)...")
    
    INSTANCES_DIR = 'agricultural_csp/instances'
    RESULTS_DIR = 'agricultural_csp/results_ttt'
    os.makedirs(RESULTS_DIR, exist_ok=True)
    
    TERMINATION_CRITERIA = TerminationCriteria(max_time_secs=600) # 10 minutos
    ...
    SOLVER_CONFIGS = {
        ...
        'TS-First-Phased': {
            'search_strategy': 'first',
            'phased_optimization': PHASED_PARAMS
        },
        'TS-Best-Phased': {
            'search_strategy': 'best',
            'phased_optimization': PHASED_PARAMS
    ...
    print(f"Metadados carregados. Selecionando {len(INSTANCES_TO_TEST)} instâncias.")
    
    tasks = []
    
    for instance_key in INSTANCES_TO_TEST:
        
        instance_name_found = None
        for meta_name in metadata.keys():
            if meta_name.startswith(instance_key):
                instance_name_found = meta_name
                break
        
        if instance_name_found is None:
            print(f"AVISO: Instância para '{instance_key}' não encontrada nos metadados. Pulando.")
            continue
            
        info = metadata[instance_name_found]
        tenure = TENURE_MAP.get(info['size'], 10)
        
        for solver_name, solver_config in SOLVER_CONFIGS.items():
            for run_idx in range(NUM_REPETITIONS):
                
                run_name = f"{instance_name_found}_{solver_name}_run{run_idx+1:02d}"
                
                strategy_config = {
                    'constructive_heuristic': ConstructiveHeuristicType.FSM_COVERAGE_PLANNER,
                    'search_strategy': solver_config['search_strategy'],
                    'phased_optimization': solver_config['phased_optimization'],
                    'tabu_radius': 3,
                    'move_min_distance': 5
                }
                ...
                task_config = {
                    'run_name': run_name,
                    'instance_to_load': instance_name_found,
    ...
    print(f"Total de {len(tasks)} tarefas geradas (4 instâncias x 4 solvers x {NUM_REPETITIONS} execuções).")
    results_list = []
    start_batch_time = time.time()
    
    # --- CORREÇÃO DO ERRO DE MEMÓRIA (PAGINAÇÃO) ---
    # max_workers = max(1, os.cpu_count() - 1) 
    max_workers = 10 # Limita manualmente para 4 workers
    ...
    print("\n" + "="*80)
    print("Processamento em Lote TTT Concluído")
    print(f"Tempo total: {(end_batch_time - start_batch_time) / 60:.2f} minutos.")
    
    results_df = pd.DataFrame(results_list)
    results_filepath = os.path.join(RESULTS_DIR, "ttt_run_results.csv")
    results_df.to_csv(results_filepath, index=False)

    print(f"Resultados salvos em: {results_filepath}")
    
    print("\nResumo dos Resultados (Agregado por Solver):")
    numeric_cols = ['Exec Time (s)', 'Initial Obj', 'Final Obj', 'Improvement (%)', 'Final Coverage (%)']
    print(results_df.groupby('Solver')[numeric_cols].mean().to_string(float_format="%.2f"))
